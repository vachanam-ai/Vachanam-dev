...
def test_non_telugu_prompt_has_absolute_output_language_rule_no_mirroring():
    ...
    for code in ("en", "hi"):
        ...
        assert "EVERYTHING you output MUST be strictly" in prompt
...
def test_telugu_prompt_switches_only_via_tool_never_text_mirroring():
    ...
    assert "Execute tool `switch_language(code)` IMMEDIATELY" in prompt
    assert "DOES NOT switch your language" in prompt
...
def test_switch_fence_and_no_revert_rule_every_language():
    ...
    for code in ("te", "en", "hi"):
        ...
        assert "Execute tool `switch_language(code)` IMMEDIATELY" in prompt
        assert "Preserve workflow state" in prompt
