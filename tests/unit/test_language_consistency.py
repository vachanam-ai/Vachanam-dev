...
def test_telugu_prompt_keeps_match_caller_but_never_mirror_directive():
    # te intentionally stays match-the-caller (Tenglish reality) — but the old
    # blanket "mirror them" wording must be gone everywhere.
    prompt = _prompt("te")
    assert "mirror them" not in prompt
    assert "MATCH THE CALLER" in prompt
