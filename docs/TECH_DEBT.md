...
| TD-028 | P3 | 2026-07-30 (comments RESOLVED 2026-07-31) | backend-engineer | **Wake-gate obsolescence under Supabase flat-compute.** RESOLVED half: every "Neon" comment across backend/agent/tests was refreshed to the Supabase rationale (cold-connect handshake + always-on pooler), NOT a blind s/Neon/Supabase/ — the false per-wake/5-min cost model was dropped, not mangled. OPEN half: the #299 wake-gate itself (Redis-gating DB queries to avoid paid wakes) still exists. | Supabase does NOT bill per wake and pauses only after ~7 idle days, so the wake-gate's original cost justification is gone; it now only trims a little idle query volume — harmless but arguably needless complexity. | Decide whether to keep the wake-gate as cheap query-load insurance or delete it (wake_gate.py + its call sites + tests) to simplify. Not blocking; no correctness risk either way. | When next touching the jobs/ scheduler or wake-gate |
...
DB role — Supabase single-role today), TD-026/034/035 (Phase 9 onboarding), TD-031
