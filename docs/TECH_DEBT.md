...
| TD-028 | P3 | 2026-07-30 | backend-engineer | **Stale "Neon" comments + wake-gate obsolescence after the Supabase migration.** ~13 backend/agent comments still explain the #299 wake-gate / maintenance-batching machinery in terms of Neon's per-wake billing + 5-min scale-to-zero (e.g. backend/jobs/{wake_gate,maintenance,calendar_writer}.py, backend/services/clinic_cache.py, backend/watchdog.py, backend/main.py inline comments, tests/unit/test_{wake_gate,watchdog,database_pool,db_outage_graceful_call}.py). The active symbols were renamed (`_db_warm_loop`/`_db_tick`); these are comment-only. | A blind s/Neon/Supabase/ would make them WRONG — Supabase does NOT bill per wake and pauses only after ~7 idle days, so the comments describe a cost model that no longer applies. Left accurate-as-history rather than mangled. | Re-evaluate whether the #299 wake-gate (query batching to avoid paid wakes) is still worth its complexity under Supabase's flat-compute model; if not, simplify/remove the wake-gate + its comments together. If kept, refresh the comments to the Supabase rationale (avoid pooler cold-connect + 7-day pause). | When next touching the jobs/ scheduler or wake-gate |
...
- **TD-027 (Alembic chain): CLOSED 2026-07-29.** `8559268c0c44` now contains only its intended audit-table/FK/index delta. A clean PostgreSQL database upgrades from base through `kk34_schema_alignment`, direct `8559` downgrade/upgrade passes, and `alembic check` reports no model/schema drift. CI runs the real empty-database upgrade on every push; ZAP no longer depends on the old create_all/stamp workaround.
