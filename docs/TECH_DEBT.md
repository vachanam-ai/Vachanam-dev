...
|---|---|---|---|---|---|---|---|
| TD-041 | P2 | 2026-08-07 | *(pending)* | WhatsApp: the model can TELL a patient their booking was moved/cancelled without calling the tool that does it. Found in a live Hindi E2E — "मेरे पिता का अपॉइंटमेंट 11:30 बजे कर दीजिए" got "मैं बदल देती हूँ" while the booking stayed at 11:00. A patient told their appointment moved stops worrying about it, so this is a correctness bug, not cosmetics. `_claims_a_mutation` + `wa_agent_unbacked_mutation_claim` DETECT it today (grep prod logs for frequency). Three fixes improved identification but not the tool call: patient name in `my_appointments`, `date` optional on reschedule, cross-script doctor/name matching. A corrective re-prompt was tried and REVERTED — it made the model apologise about its own previous message and ask the patient for the internal appointment ID. Next avenue is forcing the tool call (function-calling mode ANY on a detected mutation intent) rather than persuading the model in prose; English works, so this is Hindi-phrasing specific. Repro: `E2E_LIVE=1 pytest tests/integration/test_e2e_live_journey.py::test_multilingual_journey -s`. |
