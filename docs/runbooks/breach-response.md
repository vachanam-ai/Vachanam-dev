...
1. **Rotate the compromised database credentials immediately:**
   ...
   - Reset the database password there (Settings > Database > Reset password)
...
3. **Read the Postgres logs** (Supabase Dashboard > Logs > Postgres) to see what
   queries the attacker ran. Retention depends on plan — pull them BEFORE they age out.
...
2. **If UPDATE/DELETE succeeded** (permissions were wrong):
   ...
   - Check Supabase Dashboard > Database > Backups. PITR is a paid Supabase
     add-on and may NOT be enabled — confirm which you have before relying on it.
   - Restore audit_log from the most recent backup taken before the tamper
3. **Investigate who did it:** The tamper itself should show in Supabase's Postgres
   logs or the application logs.
