...
> Each clinic connects its own WhatsApp Business Account through Embedded
> Signup and remains the owner of that account. We act as a Technology
> Provider and send only on the WABA a clinic has explicitly connected, using
> that clinic's own credential. We never send from one clinic's account on
> behalf of another.
>
> We use this permission for two things.
>
> 1. Sending transactional appointment messages. When a patient books,
>    reschedules or cancels an appointment with the clinic, and ahead of the
>    appointment date, we send the corresponding approved UTILITY template to
>    that patient: booking confirmation, reschedule notice, cancellation
>    notice, and appointment reminder. Template content is appointment
>    logistics only — patient first name, doctor name, clinic name, date,
>    time, and token number.
>
> 2. Replying to patients inside the customer service window. Patients reply
>    in the same thread to reschedule, cancel, or ask a routine question such
>    as clinic timings or location. We respond within the 24-hour customer
>    service window opened by their message. If we cannot answer, the question
>    is passed to clinic staff, who reply to the patient themselves.
>
> We send no marketing messages. We send and request no media. We include no
> health information, diagnosis, prescription, or treatment content in any
> message.
...
**Do not claim media.** `wa_service.py` sends `template`, `text` and
`interactive` only; there is no media upload or retrieval anywhere in the
codebase. A draft generated by Meta's own assistant offered to claim we send
"doctor's notes or medical instructions" as multimedia — that is functionality
we do not have AND a health-data claim that contradicts our published
`/data-handling` page. Claiming either invites a Data Protection Assessment
over a capability a reviewer will not find.

**Do not claim we replace the clinic's WhatsApp account.** The same draft said
we "eliminate the need for clinics to have a separate WhatsApp account". That
is the inverse of our architecture — the clinic owns its WABA, which is what
Embedded Signup shows the reviewer.

Number registration and business-profile reads belong to
`whatsapp_business_management`. Keep them out of the messaging justification.
