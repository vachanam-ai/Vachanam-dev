...
|---|---|---|---|
| **Sarvam AI** | Converts your voice to text (speech-to-text) and text back to voice (text-to-speech) during your call. Audio is streamed in real time and not stored by Sarvam beyond the duration of the call. | India | [sarvam.ai/privacy](https://sarvam.ai/privacy) |
...
- **Data residency:** Voice processing (Sarvam AI), telephony (Vobiz), cache (Upstash), voice compute (Fly.io, LiveKit) all operate within India. Database (Neon) and backend API (Render) operate in Singapore. We have chosen vendors with the strongest India or near-India data residency available for each function.
