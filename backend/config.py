...
class Settings(BaseSettings):
    ...
    # Graceful DB-read failure speech (2026-07-31, Vinay: "it should never
    # answer db is unreachable"). When a read tool's DB connection fails even
    # after run_db_read's retries (a real backend outage / pooler breaker — NOT
    # a data condition), speak a fixed warm native line ("sorry, couldn't check
    # just now, shall I try again") and stop, instead of letting the LLM
    # improvise a raw "unable to fetch data from database" at the patient. This
    # is the read-side mirror of the write-side _stop_on_mutation_failure, which
